...
N,Q=MAP()
ans = [None]*Q
def lis_length(arr, query):
    global ans
    """
    最大増加部分列の長さを求める関数(実際の部分列は求めないとき)
    1. lisが単調増加となるよう2分探索でidxもとめてその数字に置き換える(ただし、末尾の場合はただの追加)
    2. 最後のlisの長さが答え
    """
    lis = []
    now = 0
    #print(query)
    for r, x, idx in query:

        while r>=now and now<len(arr):
            y = arr[now]
            ...
            if pos == len(lis):
                ...
                lis[pos] = y
            now += 1
        #print(now, lis, r,x,idx,arr)
        queryidx = bisect(lis, x)
        ans[idx] = queryidx
       # print(ans)
