...
def dfs(now, cnt):
    global ans, visited
    #print(now, cnt, ans, visited)
    
    if visited[now]:
        if ans[now] is None:
            #閉路
            circle = cnt-visited[now]
            return cnt, circle
        else:
            return cnt + ans[now], None
    else:
        visited[now] = cnt
        
        next = a[now]-1      
        anscnt, circle = dfs(next, cnt+1)
        if circle is not  None and anscnt-cnt<=circle:
            ans[now] = circle
        else:
            ans[now] = anscnt-cnt
        
        return anscnt, circle
