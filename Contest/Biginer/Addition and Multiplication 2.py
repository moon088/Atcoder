...
for i in range(9):
    cost[i] = a[i][0]
    num[i] = max(a[i][1], temp)
    temp = num[i]
basecost = cost[0]
idx = bisect(cost, cost[0])
basenum = num[idx-1]
yobun = N%basecost
keta = N//basecost
now = 0
#print(num, cost)
while now<keta:
    idx = bisect(cost, yobun+basecost)
    usenum = num[idx-1]
    useidx = bisect_left(num, usenum)
    print(usenum, end="")
    yobun -= cost[useidx]-basecost
    now+=1
print(f"{num}"*(keta-now), end="")