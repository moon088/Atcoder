...
N,D=MAP()
kabel = SortedList(key= lambda x:x[0]); kaber =[]
for i in range(N):
    l,r=MAP()
    kaber.append((l,r))
    kabel.add((l,r))
kaber.sort(key= lambda x:x[1])
...
for i in range(N):
    tempr = kaber[i]
    if tempr not in kabel: continue
    templ = tempr[1]+D-1
    cnt = bisect(kabel, templ, key=lambda x:x[0])
    del kabel[:cnt]
    ans+=1
    #print(cnt, tempr, templ,kabel,kaber)
