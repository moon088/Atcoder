...
def kruskal(n, edges,Q):
    ans = [None]*Q
    uf = UnionFind(n)
    mst_nodes = [[]*N for _ in range(N)]
    d = defaultdict(list)

    for i, (w, f, u, v,num) in enumerate(edges):
        if not uf.same(u, v):
            if f:
                ans[num] = True
                continue
            mst_nodes[u].append(i)
            mst_nodes[v].append(i)
            uf.union(u,v)
        else:
            if f:
                ans[num] = False
...
for _ in range(M):
    u,v,w=MAP()
    edges.append((w,0, u-1, v-1,None))
for i in range(Q):
    a,b,m=MAP(); a-=1; b-=1
    edges.append((m,1,a,b,i))
edges.sort(key = lambda x:x[0])

ans = kruskal(N,edges,Q)
for i in ans:
=======
# E
import sys, re
