...
nowwindow = SortedList(A[:M]+[INF])
nowsum = sum(nowwindow[:K])
now=0
while now<=N-M:
    if now!=0:
        goodbye = A[now-1]
        idx = nowwindow.bisect_right(goodbye)
        if idx<=K: 
            nowsum += nowwindow[K]
            nowsum -= goodbye
        nowwindow.discard(goodbye)
        #print(nowwindow, nowsum)
        hello = A[now+M-1]
        idx = nowwindow.bisect_right(hello)
        if idx<K:
            nowsum -= nowwindow[K-1]
            nowsum += hello
        nowwindow.add(hello)
    #print(nowwindow, nowsum)
    print(nowsum, end=" ")
