...
class Trie:
    ...
    def add(self, s: str, idx: int, isY: bool):
        k = 0
        for ch in s:
            c = ord(ch) - ord("a")
            node = self.nodes[k]
...
for i in range(Q):
    t, s = input().split()
    if t == "1":
        trie.add(s, i, isY=False)
    else:
        totalY += 1
        trie.add(s, i, isY=True)
    print(totalY - len(trie.prefixedY))
