...
length = deque([cnt for cal, cnt in run_length.encode(S) if cal=="?"])
max_circlecnt = S.count("o")
for num in length:
    if num&1: max_circlecnt+=(num+1)//2
    else: max_circlecnt+=num//2
if max_circlecnt>K:
    print("".join(S))
    exit()
