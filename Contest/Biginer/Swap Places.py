...
def dfs(g, C, N):
    visited = [[INF]*N for _ in range(N)]; visited[0][N-1]=0
    q = deque([(0, N-1)])
    while q: 
        nowx, nowy = q.popleft()
        for nearx in g[nowx]:
            for neary in g[nowy]:
                if C[nearx]!=C[neary] and visited[nearx][neary]==INF:
                    q.append((nearx, neary)) 
                    visited[nearx][neary] = visited[nowx][nowy]+1
    return visited[N-1][0] if visited[N-1][0]!=INF else -1
