...
MOD = 998244353
ifac = [None]*(3*10**6+10)
bunbofac = [None]*(3*10**6+10)
for i in range(1, len(ifac)):
    if i==1:
        ifac[i]=1
        bunbofac[i] = 1
        ifac[i-1]=1
        ...
        bunbofac[i] = (bunbofac[i-1]*pow(i, -1, MOD))%MOD

ans = 0
for b in range(B+1):
    tmp = (ifac[A+b-1]*bunbofac[b]*bunbofac[A-1])%MOD * (ifac[C+D+B-b]*bunbofac[C]*bunbofac[D+B-b])%MOD
    ans = (ans+tmp)%MOD
print(ans)
