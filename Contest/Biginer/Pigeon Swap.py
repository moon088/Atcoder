...
mapping_rev = [i for i in range(N)]
...
for i in range(Q):
    ...
    if f==1:
        a,b=x[0],x[1]
        hato[a-1] = mapping[b-1]

    elif f==2:
        a,b=x[0],x[1]
        mapping[a-1], mapping[b-1]=mapping[b-1],mapping[a-1]
        mapping_rev[mapping[a-1]]=a-1
        mapping_rev[mapping[b-1]]=b-1
        
    else:
        a=x[0]
        print(mapping_rev[hato[a-1]]+1)
